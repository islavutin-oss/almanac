...
with `{key}` placeholders filled in. Never invent a value the person did not
type.

### file

Something you generated that the reader should keep. `generate_report` writes
the artefact and returns this block; pass it through unchanged.

```file
{"name": "shortlist-20260830.csv", "url": "/api/workspace/files/abc123",
 "kind": "csv", "size": 4096, "caption": "What is in it, and what it omits."}
```
